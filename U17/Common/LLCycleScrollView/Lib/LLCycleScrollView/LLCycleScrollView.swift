...
open class LLCycleScrollView: UIView {
    // MAKR: DataSource
    open var bg_imagePaths: Array<String> = []
...
extension LLCycleScrollView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isOnlyTitle && titles.count > 0{
            ...
            // 0==count 占位图
            if imagePaths.count == 0 {
                cell.bg_imageView.image = coverViewImage
                cell.imageView.image = coverViewImage
            } else {
                let itemIndex = pageControlIndexWithCurrentCellIndex(index: indexPath.item)
                let bg_imagePath = bg_imagePaths[itemIndex]
                ...
                if imagePath.hasPrefix("http") {
                    ...
                    cell.bg_imageView.kf.setImage(with: URL(string: isAddingPercentEncodingForURLString ? escapedString ?? imagePath : imagePath), placeholder: placeHolderImage)
                    cell.imageView.kf.setImage(with: URL(string: isAddingPercentEncodingForURLString ? escapedString ?? bg_imagePath : bg_imagePath), placeholder: placeHolderImage)
                } else {
                    
                    if let image = UIImage.init(named: imagePath) {
                        cell.bg_imageView.image = image;
                    }else{
                        cell.bg_imageView.image = UIImage.init(contentsOfFile: imagePath)
                    }
                    if let image = UIImage.init(named: bg_imagePath) {
                        cell.bg_imageView.image = image;
                    }else{
                        cell.bg_imageView.image = UIImage.init(contentsOfFile: imagePath)
                    }
